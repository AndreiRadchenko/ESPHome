...
до відкриття, закриття чи зупинки воріт. Стан воріт визначається за допомогою `binary_sensor.gate_open_sensor`, до якого підключений до магнітний сенсор дверей. `binary_sensor.gate_open_sensor` має 2 секундну затримку на перехід у стан 'on'. Вона введена для фільтрації випадкових 
